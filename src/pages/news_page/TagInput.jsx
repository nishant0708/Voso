...
const TagInput = () => {
  ...
  return (
    <div>
      <label htmlFor="tag-input" className='mb-4 text-md text-black dark:text-white'>Tag:</label>
      <input
        type="text"
        id="tag-input"
        onKeyDown={addTag}
      ...
      <ul   className='flex flex-wrap'> 
        {tags.map((tag, index) => (
          <li style={{backgroundColor:"black", marginRight:"5px",color:"white", padding:"5px 15px",marginBottom:"10px"}} key={index} >
            {tag} <button style={{background:"white", color:"black",borderRadius:"99rem", padding:"1px 10px", scale:"0.6"}} onClick={() => removeTag(index)}>x</button>
          </li>
        ))}
        </ul>
        </div>
