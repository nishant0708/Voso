...
const News_page = () => {
    ...
    const [linkName, setLinkName] = useState(''); // State for the link name input
    ...
    const handleAddLink = () => {
        ...
        const existingLinkIndex = links.findIndex(link => link.name === selectedSource);
        ...
        <DefaultLayout>
            <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark p-5">
                ...
                <div>
                    ...
                    <div style={{ display: type === 'article' ? 'none' : 'block' }} className="mb-4 text-md text-black dark:text-white">
                    ...
                    <div style={{ display: type === 'article' ? 'none' : 'block' }}>
                        {show ? (
                            <>
                                <p className="mb-4 text-bold text-lg text-black dark:text-white">Links:</p>
                                ...
                                <button className='bg-primary text-white px-2 py-1' onClick={() => setshow(prev => !prev)}>Add Links</button>
                            </>
                        ) : (
                            <div className="mb-4 text-md text-black dark:text-white">
                                <p>Add Links:</p>
                                <div className='mb-3 flex flex-col gap-2.5 md:gap-6 md:flex-row'>
                                    ...
                                    <div className='w-full md:w-1/2' >
                                        <div  className="mb-4 text-md text-black dark:text-white ">
                                            ...
                                            >
                                                ...
                                                <option value="HTDS Content Services">HTDS Content Services</option>
                                                <option value="Daily Prabhat">Daily Prabhat</option>
                                                <option value="Mumbai Times">Mumbai Times</option>
                                                <option value="Indian News Network">Indian News Network</option>
                                                <option value="Indian Economic Observer">Indian Economic Observer</option>
                                    ...
                                    <input
                                        ...
                                        className='relative w-full md:w-1/2 z-20 h-10 mt-1.5 text-sm text-black dark:text-white w-full appearance-none rounded border border-stroke bg-transparent py-0.5 px-3 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary'
                                    />
                                </div>
                                <button className='bg-primary text-white px-2 py-1' onClick={handleAddLink}>Confirm</button>
                            </div>
                        )}
                    </div>
                    <button className='mt-6 w-full bg-primary text-white px-2 py-1' >Save Changes</button>
                </div>
            </div>
        </DefaultLayout>
    );
